...
public class TaoProcessor implements Processor {
	...
	public void answerRequest(ClientRequest req, ServerResponse resp, boolean isFakeRead) {
		...
		// case if the real read returned before this fake read
		...
		// unlocking this earlier causes a concurrency issue perhaps having to do with
		// mResponseMap
		mSubtreeRWL.writeLock().unlock();
		...
		if (!fakeRead) {
			...
			while (!requestList.isEmpty()) {
				...
				if (currentRequest.getType() == MessageTypes.CLIENT_WRITE_REQUEST) {
					...
					// If elementDoesExist == false and the request is not a write, we will not put
					// assign this block ID
	...
	public byte[] getDataFromBlock(long blockID) {
		TaoLogger.logDebug("Trying to get data for blockID " + blockID + "\nI think this is at path: "
				+ mPositionMap.getBlockPosition(blockID));
		...
		while (true) {
			...
			if (targetBucket != null) {
				...
				if (targetBlock != null) {
					...
					TaoLogger.logDebug("scuba Cannot find block " + blockID + " in subtree or stash");
					// System.exit(1);
					// continue;
				}
			}
